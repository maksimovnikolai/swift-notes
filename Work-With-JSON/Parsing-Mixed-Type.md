## Parsing Mixed Types
* postcode иногда представляет собой Int или String, 
* это приведет к ошибкам декодирования при синтаксическом анализе
...
```swift
// Структура данных JSON верхнего уровня представляет собой словарь.
...
enum AppError: Error {
  case missingValue // если не Int или String, выдайте эту ошибку
}

// создание кастомного объекта, который будет типом данных postcode
enum Postcode: Decodable {
// обрабатываем два случая в свойстве JSON "postcode"
// где почтовый индекс может быть либо (Int), либо (String)
 
// мы также будем использовать ассоциативные значения для фиксации
// значение почтового индекса
// например, если это Int, мы захватим значение Int
  case int(Int)
  case string(String)
  
  // overridbe the init(from decoder:) initializer
  init(from decoder: Decoder) throws {
    // обрабатывать два случая Int или String
    
    // если это не Int или String, мы выдадим
    // error
    ...
    throw AppError.missingValue // не было Int или String
  }
}
```
